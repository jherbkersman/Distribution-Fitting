CF.center <- function (data, discrete = FALSE, boot = NULL, method = "unbiased", 
                        graph = TRUE, obs.col = "darkblue", obs.pch = 16, boot.col = "orange") 
{
  ...
  if (graph) {
    ...
    ymax <- kurtmax - 1
    plot(skewdata^2, kurtmax - kurtdata, pch = obs.pch, xlim = c(0, 
                                                                 xmax), ylim = c(0, ymax), yaxt = "n", xlab = "square of skewness", 
         ylab = "kurtosis", main = "Cullen and Frey graph")
    ...
    if (!discrete) {
      ...
      if (!is.null(boot)) {
        ...
        legend(xmax * 0.2, ymax * 0.93, pch = 13, legend = "boot cluster center", 
               bty = "n", cex = 0.8, pt.cex = 1.2, col = 'red')
      }
    ...
    if (!is.null(boot)) {
      points(s2boot, kurtmax - kurtboot, pch = 1, col = boot.col, 
             cex = 0.5)
      bootpoints <- cbind(s2boot, (kurtmax-kurtboot))
      points(mean(s2boot), mean(kurtmax-kurtboot), pch=13, col='red', cex=1.5)
    }
